...
class Model(object):
    ...
    def config_model(self):
        ...
        if use_input_dropout:
            ...
            cell = tf.tf.compat.v1.nn.rnn_cell.DropoutWrapper(cell, input_keep_prob=self.hps.input_dropout_prob)

        if use_output_dropout:
            tf.compat.v1.logging.info('Dropout to output w/ keep_prob = %4.4f.', self.hps.output_dropout_prob)
    ...
    def build_RPCL_pix2seq(self):
        ...
        self.q_mu, self.q_sigma2, self.q_alpha \
            = tf.cond(pred=tf.cast(self.global_ > 500, tf.bool),
                      true_fn=lambda: tf.cond(pred=tf.cast(self.global_ % 3, tf.bool),
                                      true_fn=lambda: self.em(self.p_mu, self.p_sigma2, self.p_alpha, self.de_mu, self.de_sigma2, self.de_alpha),
                                      false_fn=lambda: self.rpcl(self.p_mu, self.p_sigma2, self.p_alpha, self.de_mu, self.de_sigma2, self.de_alpha)),
        ...
        self.update_gmm_sigma2 = tf.compat.v1.assign(self.de_sigma2, self.q_sigma2)
        self.update_gmm_alpha = tf.compat.v1.assign(self.de_alpha, self.q_alpha)
    ...
    def calculate_prob(self, x, q_mu, q_sigma2):
        ...
        log_exp_part = -0.5 * tf.reduce_sum(input_tensor=tf.divide(tf.square(x - mu), 1e-30 + sigma2), axis=2)
        log_frac_part = tf.reduce_sum(input_tensor=tf.math.log(tf.sqrt(sigma2 + 1e-30)), axis=2)
        log_prob = log_exp_part - log_frac_part - float(self.hps.z_size) / 2. * tf.math.log(2. * 3.1416)
        return tf.exp(tf.cast(log_prob, dtype=tf.float64))
    ...
    def calculate_posterior(self, y, q_mu, q_sigma2, q_alpha):
        ...
        temp = tf.multiply(tf.tile(tf.transpose(a=tf.cast(q_alpha, dtype=tf.float64), perm=[1, 0]), [self.hps.batch_size, 1]), prob)
        sum_temp = tf.tile(tf.reduce_sum(input_tensor=temp, axis=1, keepdims=True), [1, self.k])
        gamma = tf.clip_by_value(tf.cast(tf.divide(temp, 1e-300 + sum_temp), dtype=tf.float32), 1e-5, 1.)
        gamma_st = gamma / (1e-10 + tf.tile(tf.reduce_sum(input_tensor=gamma, axis=1, keepdims=True), [1, self.k]))
        return gamma_st, tf.argmax(input=gamma_st, axis=1)
    ...
    def rpcl(self, y, en_sigma2, gamma, q_mu_old, q_sigma2_old, q_alpha_old):
        ...
        with tf.compat.v1.variable_scope('rpcl'):
            penalize = 1e-4  # De-learning rate
            temp_y = tf.tile(tf.expand_dims(y, axis=1), [1, self.k, 1])
            winner = tf.one_hot(tf.argmax(input=gamma, axis=1), self.k, axis=1)  # the winner
            rival = tf.one_hot(tf.argmax(input=gamma - gamma * winner, axis=1), self.k, axis=1)  # the rival
            gamma_rpcl = winner - penalize * rival
            sum_gamma_rpcl = tf.tile(tf.expand_dims(tf.reduce_sum(input_tensor=gamma_rpcl, axis=0), axis=1), [1, self.hps.z_size])
            q_mu_new = tf.reduce_sum(input_tensor=temp_y * tf.tile(tf.expand_dims(gamma_rpcl, axis=2),
                                                      [1, 1, self.hps.z_size]), axis=0) / (sum_gamma_rpcl + 1e-10)
            q_sigma2_new = tf.reduce_sum(input_tensor=(tf.square(temp_y - tf.tile(tf.expand_dims(q_mu_new, axis=0), [self.hps.batch_size, 1, 1])) + en_sigma2)
                                         * tf.tile(tf.expand_dims(gamma_rpcl, axis=2), [1, 1, self.hps.z_size]), axis=0) \
                           / (sum_gamma_rpcl + 1e-10)
            q_alpha_new = tf.expand_dims(tf.reduce_mean(input_tensor=gamma_rpcl, axis=0), axis=1)
            ...
            q_alpha_st = q_alpha / tf.reduce_sum(input_tensor=q_alpha)
    ...
    def em(self, y, en_sigma2, gamma, q_mu_old, q_sigma2_old, q_alpha_old):
        ...
        with tf.compat.v1.variable_scope('em'):
            sum_gamma = tf.tile(tf.expand_dims(tf.reduce_sum(input_tensor=gamma, axis=0), axis=1), [1, self.hps.z_size])
            temp_y = tf.tile(tf.expand_dims(y, axis=1), [1, self.k, 1])

            q_mu_new = tf.reduce_sum(input_tensor=temp_y * tf.tile(tf.expand_dims(gamma, axis=2), [1, 1, self.hps.z_size]), axis=0) / (sum_gamma + 1e-10)
            q_sigma2_new = tf.reduce_sum(input_tensor=(tf.square(temp_y - tf.tile(tf.expand_dims(q_mu_new, axis=0), [self.hps.batch_size, 1, 1])) + en_sigma2)
                                         * tf.tile(tf.expand_dims(gamma, axis=2), [1, 1, self.hps.z_size]), axis=0) / (sum_gamma + 1e-10)
            q_alpha_new = tf.expand_dims(tf.reduce_mean(input_tensor=gamma, axis=0), axis=1)
    ...
    def calculate_deconv_loss(self, img, gen_img, sign):
        ...
        if sign == 'square':
            return tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=tf.square(img - gen_img), axis=1))
        elif sign == 'absolute':
            return tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=tf.abs(img - gen_img), axis=1))
        else:
            assert False, 'please choose a respectable cell'
    ...
    def get_lil_loss(self, pi, mu1, mu2, s1, s2, corr, pen_logits, x1_data, x2_data, pen_data):
        ...
        result2 = tf.nn.softmax_cross_entropy_with_logits(logits=pen_logits, labels=tf.stop_gradient(pen_data))
        result2 = tf.reshape(result2, [-1, 1])

        if not self.hps.is_training:
            result2 = tf.multiply(result2, masks)
        return tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=tf.reshape(result1 + result2, [self.hps.batch_size, -1]), axis=1))
